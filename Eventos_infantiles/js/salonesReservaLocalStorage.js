...
// FUNCIÓN: Muestra los salones en el catálogo
function renderCatalog(data) {
  const container = document.getElementById("catalogo-container");
  if (!data || data.length === 0) {
    container.innerHTML = `<div class="col-12 text-center py-5"><h4>No hay salones disponibles con estos filtros</h4></div>`;
    return;
  }
    ...
    <div class="col-lg-4 col-md-6 mb-4">
      <div class="card h-100 salon-card">
        <div class="position-relative overflow-hidden rounded-top">
          <img src="${item.imagen}" class="card-img-top salon-image" alt="${item.nombre}" onerror="this.src='${PLACEHOLDER_IMAGE}'">
        ...
        <div class="card-footer bg-transparent border-0 pt-0 pb-3">
          ...
          <button class="btn btn-reservar w-100 py-2" onclick="reservar('${item.id}', ${item.capacidad}, '${item.nombre}','${item.precio}')">
            <i class="bi bi-calendar-check me-2"></i> Reservar ahora
          </button>` : ''}
          <button class="btn btn-presupuesto w-100 py-2 ${localStorage.getItem('admin') !== null ? 'mt-2' : ''}" onclick="solicitarPresupuesto('${item.id}', ${item.capacidad}, '${item.nombre}','${item.precio}')">
...
function reservar(salonId, capacidad, nombreSalon, precioSalon) {
  salonSeleccionado = { id: salonId, capacidad: parseInt(capacidad), nombre: nombreSalon, precio: parseFloat(precioSalon) };
...
function calcularPrecioTotal() {
  ...
  if (cantidadPersonas > capacidadSalon) {
    document.getElementById('errorCapacidad').textContent = `La cantidad de personas (${cantidadPersonas}) supera la capacidad del salón (${capacidadSalon})`;
  ...
  if (incluyeMaquillaje) {
    ...
    servicios.push({ nombre: "Maquillaje", costo: costoMaquillaje.toFixed(2) });
  }
  if (incluyeCatering) {
    const costoCatering = precioBase * 0.002 * cantidadPersonas;
    precioTotal += costoCatering;
    servicios.push({ nombre: "Catering", costo: costoCatering.toFixed(2) });
  }
...
async function guardarReserva(reservaData) {
  try {
    ...
    const reservaExistente = reservas.find(r => r.salonId === reservaData.salonId && r.fechaEvento === reservaData.fechaEvento);
    if (reservaExistente) {
...
function calcularPrecioTotalPresupuesto() {
  ...
  if (cantidadPersonas > capacidadSalon) {
    document.getElementById('errorCapacidadPresupuesto').textContent = `La cantidad de personas (${cantidadPersonas}) supera la capacidad del salón (${capacidadSalon})`;
...
// --- FUNCIONES Y EVENT LISTENERS GENERALES ---
...
// *** INICIALIZACIÓN DE EVENTOS (CORREGIDO) ***
document.addEventListener("DOMContentLoaded", () => {
  // Carga los datos de los salones al iniciar
  fetchData();
  ...
  document.getElementById('btnConfirmarReserva').addEventListener('click', async function() {
    ...
    if (!reservaData.cliente || !reservaData.telefono || !reservaData.fechaEvento) {
      return mostrarError('Por favor complete todos los campos obligatorios');
    }
    if (reservaData.cantidadPersonas <= 0) {
      return mostrarError('La cantidad de personas debe ser mayor a cero');
    }
    if (reservaData.cantidadPersonas > salonSeleccionado.capacidad) {
      return mostrarError(`La cantidad de personas supera la capacidad del salón`);
    }
    if (!validarNombre(reservaData.cliente)) {
      return mostrarError('El nombre solo puede contener letras y espacios');
    }
    if (!validarTelefono(reservaData.telefono)) {
      return mostrarError('El telefono no permite letras ni caracteres especiales');
    }
    ...
    if (resultado) {
      ...
      window.location.href = '/pages/verReservaLocalStorage.html';
    }
  });

  // --- Listeners para el modal de PRESUPUESTO ---
  ...
  document.getElementById('btnConfirmarPresupuesto').addEventListener('click', async function() {
    ...
    if (!validarTelefono(presupuestoData.telefono)) {
      return mostrarError('El teléfono debe contener solo números (8-15 dígitos).');
    }
